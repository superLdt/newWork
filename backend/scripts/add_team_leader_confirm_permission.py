...
def add_team_leader_confirm_permission():
    """为班组长角色添加supplier:confirm权限"""
    app = create_app()
    with app.app_context():
        print("开始为班组长角色添加supplier:confirm权限...")
        
        # 查找班组长角色
        team_leader_role = Role.query.filter_by(name='班组长').first()
        if not team_leader_role:
            print("✗ 未找到班组长角色")
            return False
        
        print(f"✓ 找到班组长角色: {team_leader_role.name} (ID: {team_leader_role.id})")
        ...
        if existing_role_permission:
            print("✓ 班组长角色已有supplier:confirm权限，无需重复添加")
            return True
        ...
        try:
            ...
            print("✓ 成功为班组长角色添加supplier:confirm权限")
...
if __name__ == '__main__':
    success = add_team_leader_confirm_permission()
    if success:
        print("\n权限配置完成！班组长现在可以进行任务确认操作。")
    else:
        print("\n权限配置失败！请检查错误信息。")
        sys.exit(1)